...
def map_submission_tasks(submission):
    ...
    system_prompt = (
        ...
        "14. PRIORITISE DIRECT VISUAL EVIDENCE WHEN THE TASK ASSESSES A VISIBLE "
        "ARTEFACT: For tasks that assess the presence, quality, layout, formatting, "
        "behaviour, configuration, usability, or implementation of a visible artefact, "
        "prioritise pages that show the artefact itself. A learner's written description "
        "of an artefact must not replace available direct visual evidence of that artefact. "
        "When both the written explanation and the visual artefact are materially relevant, "
        "map both so the grading stage can consider them together.\n"
