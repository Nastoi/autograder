...
def map_submission_tasks(submission):
    ...
    system_prompt = (
        ...
        "14. DIRECT VISUAL EVIDENCE FOR OBSERVABLE REQUIREMENTS: When a task "
        "assesses the presence, appearance, layout, formatting, configuration, "
        "behaviour, interaction, or quality of a visible artefact, written descriptions "
        "or claims are supporting evidence only. They do not verify that the artefact "
        "or behaviour was actually implemented. Map the page where the artefact itself "
        "is visibly shown whenever such evidence exists. If the submission contains "
        "only a written description and no direct visual evidence for an observable "
        "requirement, the written page may still be mapped as supporting evidence, but "
        "do not treat it as proof of implementation. State this limitation clearly in "
        "the mapping justification.\n"
...
def grade_submission(submission):
    ...
    system_prompt = (
        ...
        "For observable requirements, distinguish between a learner's written claim "
        "and direct evidence of implementation. A written description may demonstrate "
        "understanding or intent, but it does not by itself verify that a dashboard, "
        "visual, slicer, interaction, configuration, or other observable feature was "
        "actually implemented. When direct evidence required to verify the feature is "
        "absent, award credit only for what the written evidence supports and identify "
        "the missing verification in the feedback. "
        ...
        "If score_percentage is below 100, the feedback MUST end with at least one "
        "specific and practical improvement action that tells the learner exactly "
        "what to change, add, correct, verify, or demonstrate. A feedback response "
        "that identifies a weakness without giving a concrete action is incomplete."
        "Do not use vague advice such as 'improve clarity', 'provide more detail',"
        "'requires refinement', or 'needs further development' unless you immediately explain how. "
